...
class CacheSetState:
    ...
    def must_update(self, b: Hashable):
        assert not self.evicted_line, "Must analysis only supports non-evicted cache set state."
        b_age = self.where(b)
        b_age = b_age if b_age != -1 else self.max_age
        for age in reversed(range(0, b_age)):
            self.age_shifting(age)
        self.new_access_insert(b)

    def may_update(self, b: Hashable):
        assert not self.evicted_line, "May analysis only supports non-evicted cache set state."
        b_age = self.where(b)
        b_age = b_age if b_age != -1 else self.max_age - 1
        for age in reversed(range(0, b_age + 1)):
            self.age_shifting(age)
        self.new_access_insert(b)

    def persistent_update(self, b: Hashable):
        assert self.evicted_line, "Persistent analysis only supports evicted cache set state."
        b_age = self.where(b)
        b_age = b_age if b_age != -1 else self.max_age
        for age in reversed(range(0, b_age)):
            self.age_shifting(age)
    ...
    def must_join(self, s2):
        assert (not self.evicted_line) and (not s2.evicted_line), "Must analysis only supports non-evicted cache set state."
        assert self.max_age == s2.max_age, "Un-matched max age for two cache set state."
        new_s = CacheSetState(self.max_age, evicted=False)
        blocks = set(self.all_blocks()).intersection(set(s2.all_blocks()))
        for b in blocks:
            age1, age2 = self.where(b), s2.where(b)
            new_s.states[max(age1, age2)].add(b)
        return new_s

    def may_join(self, s2):
        assert (not self.evicted_line) and (not s2.evicted_line), "Must analysis only supports non-evicted cache set state."
        assert self.max_age == s2.max_age, "Un-matched max age for two cache set state."
        new_s = CacheSetState(self.max_age, evicted=False)
        blocks = set(self.all_blocks()).union(set(s2.all_blocks()))
        for b in blocks:
            age1, age2 = self.where(b), s2.where(b)
    ...
    def persistent_join(self, s2):
        assert self.evicted_line and s2.evicted_line, "Persistent analysis only supports evicted cache set state."
        assert self.max_age == s2.max_age, "Un-matched max age for two cache set state."
        new_s = CacheSetState(self.max_age, evicted=True)
        blocks = set(self.all_blocks()).union(set(s2.all_blocks()))
        for b in blocks:
            age1, age2 = self.where(b), s2.where(b)
            new_s.states[max(age1, age2)].add(b)
    ...
    def state_equal(self, s2):
        assert (self.max_age == s2.max_age) and (self.evicted_line == s2.evicted_line)
        for l1, l2 in zip(self.states, s2.states):
            if l1 != l2:
                return False
        return True
